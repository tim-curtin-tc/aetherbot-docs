---
title: "Core: Academy Entity"
slug: domain/entities/academy
description: Persistent representation of an Academy – the institutional node where students, aetherbots, and progression systems converge in the aetherbot domain.
status: experimental
...
# Academy (Core Domain Entity)

## 1. Canonical Definition

An **Academy** is the primary institutional aggregate in the aetherbot domain that anchors structured progression, curriculum orchestration, rostered participation (students + aetherbots), and regulated access to tournaments, training threads, and capability unlocks. It acts as both:
...
| Roster Aggregation     | Owns membership enrollment of Students (participants / administrators) and indirectly associated Aetherbots. |
...
| Student Readable Key | Slug (immutable once published; must be unique domain-wide).               |
...
| Student                    | Enrolls                   | 0..N                       | Via membership relation (AcademyMembership).            |
| Aetherbot                  | Indirect Association      | 0..N                       | Through Teams or Collections tied to enrolled Students. |
...
POST   /academies/{academyId}/members (enroll student)
DELETE /academies/{academyId}/members/{studentId}
...
- Enrollment operations must check both Academy status and target Student account status.
...
| Multi-Academy Affiliation | Allow a Student to belong to multiple simultaneously? If yes, gating conflicts?   |
