...
description: Structured competitive event container orchestrating phases, brackets, eligibility, and resolution outcomes in the aetherbot domain.
status: experimental
...
A **Tournament** is a persistent event aggregate coordinating registration, seeding, scheduling, bracket or pool phases, progression advancement, adjudication, and result finalization. It links eligible Teams / Collectors, enforces rulesets, and emits structured outcomes (rankings, stats, advancement tokens).
...
| Collector         | Registers (solo mode) | 0..N                          | If individual format supported.    |
