...
description: Coordinated multi-participant competitive unit aggregating Collectors and Aetherbots with roster governance in the aetherbot domain.
status: experimental
...
A **Team** is a structured aggregation of Collectors (participants) and their configured Aetherbots operating as a competitive unit in events or tournaments. It encapsulates roster constraints, affiliation metadata (e.g., Academy alignment), and coordination identifiers (captain, strategy notes). Teams may be ephemeral (event-specific) or persistent.

## 2. Primary Responsibilities

| Responsibility        | Description                                           |
| --------------------- | ----------------------------------------------------- |
| Roster Aggregation    | Manages member Collectors and assigned Aetherbots.    |
...
| captainCollectorId | ID(Collector) | 0..1 | Must be active member.                     |
...
| ----------------- | --------------- | ----------------------- | ---------------------------- |
| Collector         | Members         | 0..N                    | Via team membership pivot.   |
...
| ---------------------------------------- | ----------------------------------- | ------------------------------ |
| captainCollectorId must be active member | Prevent invalid leadership pointers | Transactional guard            |
...
| TeamCaptainChanged | captainCollectorId mutation   |
...
DELETE /teams/{teamId}/members/{collectorId}
```
