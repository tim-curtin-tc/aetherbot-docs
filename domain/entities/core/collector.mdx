...
| userName          | string      | 1    | Account username for login and @mentions.             |
| name              | string      | 1    | Real name or preferred display name.                  |
...
GET    /card-collectors/summary                          # Returns CardCollectorSummary[]
GET    /card-collectors/summary/academy/{academyId}      # Filtered summaries by academy
...
A lightweight representation used for list views and dashboards, containing:

- `collectorId`: Unique identifier
- `userName`: Login username
- `name`: Display name or real name
- `avatarImageUrl`: Profile image URL
- `status`: Current status (ACTIVE, SUSPENDED, RETIRED)
- `createdAt`: Timestamp of account creation
- `academyId`: Associated academy identifier

The summary endpoints optimize for performance by returning only essential fields needed for table views and collector selection interfaces.
