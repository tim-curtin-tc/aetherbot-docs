...
summary: Persistent representation of an authenticated actor account in the aetherbot domain governing identity, roles, and ownership surfaces.
status: experimental
...
An **Account** is the root authentication and authorization principal in the aetherbot domain. It owns credentials, role assignments, and acts as the administrative container for subordinate domain objects (e.g., Academy ownership, Aetherbot custody, Collection / Card ownership). It is distinct from an in-universe "Human" participant profile (which may extend behavioral or narrative attributes) but may map 1:1 in most present use cases.
...
| Display Handle         | Unique handle (policy: lowercase alnum + hyphen, length 3–40).                                       |
...
| Human             | Realizes     | 0..1                       | Optional enriched participant profile.                      |
